...
async function revealMore(delta) {
  const current = getAllTurns();
  const currentVisible = current.length;
  const target = Math.max(1, currentVisible + delta);
  
  debug(`RevealMore: delta=${delta}, current=${currentVisible}, target=${target}`);
...
async function revealExact(targetVisible) {
  ...
  const current = getAllTurns();
  let need = Math.max(0, targetVisible - current.length);
